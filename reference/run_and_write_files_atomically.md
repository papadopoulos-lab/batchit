...
But instead of `fn`'s return value crossing back to you, batchit writes
the files you declared in `outputs` and hands back a small record of
what it wrote.
...
- outputs:
  ...
  name `outputs` the same way, with the same names in any order). Each
...
item id. Each element describes what was written, as
...
declared output a `"staged_writer"` never wrote, or a failure
serializing a staged file. In each case NO declared output is touched,
and whatever was already at those paths (or nothing, if they didn't
exist) is left untouched. The item's own previous marker is removed
before the first replacement, so it does not survive even here.
...
An individual file is not left half-written, since rename replacement is
...
of them when they do not. Don't create, read, or rely on a file with
...
#>   [1/2] complete  18:22:00
#>   [2/2] complete  18:22:00
result
#> $`1`
#> $`1`$committed
#>                                           squared 
#> "/tmp/RtmpkNdNX4/batchit-return-example/sq_1.qs2" 
#>                                           doubled 
#> "/tmp/RtmpkNdNX4/batchit-return-example/db_1.qs2" 
#> 
#> $`1`$attempt
#> [1] "1b746eb1b2ed"
#> 
...
#> $`2`$committed
#>                                           squared 
#> "/tmp/RtmpkNdNX4/batchit-return-example/sq_2.qs2" 
#>                                           doubled 
#> "/tmp/RtmpkNdNX4/batchit-return-example/db_2.qs2" 
#> 
#> $`2`$attempt
#> [1] "1b743bb9517"
#> 
