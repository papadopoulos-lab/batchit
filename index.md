...
<div class="rw-card"><div class="rw-card-num">03</div><h3>Or hand the work to a scheduler</h3><p><code>slurm_it()</code> describes one Slurm job. <code>slurm_write()</code> turns a list of them into one bash file per job, plus a <code>submit.sh</code> that chains them with <code>--dependency=afterok</code>. <code>slurm_write()</code> submits nothing. <code>slurm_submit()</code> runs that driver, so you read <code>submit.sh</code> first.</p></div>
</div>
