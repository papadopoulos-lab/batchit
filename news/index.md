...
up to three places at once — `README.md`, the “Choosing a dispatch
function” vignette, and the `@section Advanced:` blocks of the help
pages — which is three copies to keep true. Each piece now has one home.
...
quick start, the which-function-do-I-want table, and links — and hands
off the depth to the vignette, which already covered all of it in more
...
are all in the vignette. One clause was genuinely missing from it and
has been added back: that the in-flight bound covers produced item
arguments only, and that `ids`, `outputs` and the accumulating result
list stay resident in the parent session for the whole call.
...
in no user-facing document: not the README, not the vignette, and its
...
`warnings = character()`, and the frontends surface warnings only for
items that inspected OK, so those warnings never become R warnings. They
...
`hexSticker`. The motif is item bars across worker lanes, ending ragged
because a worker takes the next item as soon as it is free. A local
...
redirect note went away, and `pkgdown/`, `dev/`, `index.md` and
`Rplots.pdf` are all `.Rbuildignore`d so no non-standard top-level file
ships.
