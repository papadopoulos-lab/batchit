...
test_that(".batch_log_tail() is bounded and survives non-text worker output", {
  ...
  expect_equal(batchit:::.batch_log_tail(p("small.log")), "a\nb\nreal-error-here")

  # missing / empty: nothing to say, and no error
  expect_equal(batchit:::.batch_log_tail(p("nope.log")), "")
  file.create(p("empty.log"))
  expect_equal(batchit:::.batch_log_tail(p("empty.log")), "")

  # no trailing newline: the last line is still reported
  cat("x\nlast-no-newline", file = p("nonl.log"))
  expect_match(batchit:::.batch_log_tail(p("nonl.log")), "last-no-newline")
  ...
  expect_match(batchit:::.batch_log_tail(p("nul.log")), "ERROR")
  ...
  expect_lt(nchar(batchit:::.batch_log_tail(p("oneline.log"))), 100000L)
})

test_that(".batch_log_tail() reads by seek, never by readLines()", {
  ...
  start <- grep("^\\.batch_log_tail <- function", src)
  ...
  expect_equal(grep("readLines\\(", code, value = TRUE), character(0),
    info = "readLines() reads the WHOLE file; .batch_log_tail must seek to the end")
  expect_gt(length(grep("seek\\(", code)), 0L)
  expect_gt(length(grep("readBin\\(", code)), 0L)
})

test_that(".batch_log_tail() bounds the READ, not merely the output", {
  ...
  elapsed <- system.time(out <- batchit:::.batch_log_tail(path))[["elapsed"]]
...
test_that("a successful worker's log is reclaimed as it completes, not at pool exit", {
  ...
  # those equal whether or not the per-item unlink exists. A many-item stage
  # deferring reclaim to pool exit would sit on all the log files at once.
  skip_on_cran()
  skip_if_not(.log_tail_have_tree, "package source tree not available")
