# Shape B of the ONE contract: the parent is the producer, items are generated
# lazily under backpressure, and a bounded number are in flight at once. This is
...
test_that("batch_stream() preserves a NULL result in place (results[pos] <- list())", {
  ...
  # follows to re-extend, so the result comes back length n-1. The NULL id is
...
test_that("batch_stream() never dispatches on mirai's DEFAULT compute profile", {
  ...
  # batch_stream must claim its own and leave the default alone.
...
test_that("two back-to-back batch_stream calls each get a fresh, usable profile", {
  skip_on_cran()
  skip_if_not(have_tree, "package source tree not available")
  # The runner allocates a fresh private profile per invocation (a session-local
  # counter under a session nonce) and tears it down on exit. This guards the
  # failure mode a fixed profile name would create: the first call's torn-down
  # profile leaving stale state that blocks the second call's daemons() -- or a
  # name collision. Two sequential calls must both succeed and return correct
  # results.
  r1 <- batchit::batch_stream(
    mk(".batch_fixture_echo"),
    ids = c("a", "b"), producer = function(id) list(x = id),
    n_workers = 1L, dev_path = dev_tree
  )
  r2 <- batchit::batch_stream(
    mk(".batch_fixture_echo"),
...
test_that("batch_stream() validates ids, config and dev_path before doing any work", {
  # duplicate ids
  expect_error(
    batchit::batch_stream(mk(".batch_fixture_echo"), ids = c("a", "a"),
      producer = function(id) list(x = id), n_workers = 1L),
    "unique")
  # NA / empty id
  expect_error(
    batchit::batch_stream(mk(".batch_fixture_echo"), ids = c("a", NA),
      producer = function(id) list(x = id), n_workers = 1L),
    "non-empty, non-NA")
  # timeout / collect validated as scalars, not silently disabled
  expect_error(
    batchit::batch_stream(mk(".batch_fixture_echo"), ids = "a",
  ...
  expect_error(
    batchit::batch_stream(mk(".batch_fixture_echo"), ids = character(0),
      producer = function(id) list(x = id), n_workers = 1L,
