...
Two more exports belong to a different execution shape, and section 8 covers them.
`slurm_it()` describes one Slurm job. `slurm_write()` writes a chain of those jobs
as bash. Neither takes `fn` or `items`.
...
third describes the work for a scheduler and stops there.
...
| a chain of scheduler jobs | a compute node, later | `slurm_it()`, `slurm_write()` |

The third shape breaks the pattern of the first two, and the break is deliberate.
`slurm_it()` and `slurm_write()` take no `fn` and no `items`, so the definition of
an item in section 1 does not reach them. They serialise no closure, hash no code,
and check nothing against a function's formals. There is no envelope and no
...
**Nothing submits.** A person reads `submit.sh` and runs it. batchit exports no
function that calls `sbatch`, and `R/slurm.R` starts no subprocess at all.

Do not add a function that submits. A submission costs real cluster time, and a
person cannot undo it. A manual step also keeps batchit testable off a cluster:
every Slurm test drives generated text, and not one of them needs a scheduler.
...
**No submission.** `slurm_write()` writes a Slurm chain and stops. A person runs
`submit.sh`. Section 8 gives the two reasons.
